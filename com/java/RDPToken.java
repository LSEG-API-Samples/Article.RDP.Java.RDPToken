...
public class RDPToken {
	...
	//alwayRequest is true when access token is used for Refinitiv Real-Time - Optimized(always request new access token)
	//alwayRequest is false when access token is used for RDP (return non-expired access token;otherwise request a new one)
	...
	private static String requestToken(List<NameValuePair> params, boolean useRefreshToken) throws Exception {
		...
		String RDP_VERSION_AUTH = "v1";
		String url = "https://api.refinitiv.com/auth/oauth2/" +RDP_VERSION_AUTH+ "/token";
	...
	public static void main(String[] args) {
		if(args.length!=3) {
			System.out.println("Usage: java com.java.RDPToken <username> <clientId> <alwaysRequest>" );
			System.exit(-1);
		} else {
